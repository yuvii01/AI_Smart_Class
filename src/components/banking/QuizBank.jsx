...
const QuizBank = () => {
  const [paperType, setPaperType] = useState('ibps po');
  ...
  const [input, setInput] = useState("");
    ...
    async function run6(exam, subject, topic, difficulty, numQuestions) {
  const papergene = `
You are an expert question setter for Indian banking exams (IBPS PO, SBI Clerk, RBI Assistant, etc.).
Generate a high-quality, exam-level multiple-choice quiz for the "${exam}" in the subject of "${subject}"${topic ? `, specifically focusing on the topic: "${topic}"` : ''}.
Requirements:
- The quiz must contain exactly ${numQuestions} unique, original MCQs, strictly following the latest syllabus and pattern for this exam and subject.
- Mix conceptual, application-based, and tricky questions to reflect real exam standards and trends.
- Distribute questions across important topics and difficulty levels (easy, medium, hard) as per the actual exam.
...
- Use only plain text math notation (e.g., x^2 for "x squared", sqrt(x) for square root).
- Each question and its options should not exceed 5 lines total.
- Begin the quiz with a centered heading: "${exam.toUpperCase()} – ${subject.toUpperCase()}${topic ? ` | Topic: ${topic}` : ''} | Difficulty: ${difficulty.charAt(0).toUpperCase() + difficulty.slice(1)}"
...
`;
  ...
  return (
    <Container>
      <Title>Banking Quiz Generator</Title>
